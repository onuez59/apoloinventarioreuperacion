...
class CTypesData(object):
    ...
    def _addr_repr(self, correo):
        if correo == 0:
            return 'NULL'
        else:
            if correo < 0:
                correo += 1 << (8*ctypes.sizeof(ctypes.c_void_p))
            return '0x%x' % correo
    ...
    def _convert_to_correo(self, BClass):
        if BClass is None:
            raise TypeError("cannot convert %r to an correo" % (
                self._get_c_name(),))
        else:
    ...
    def _cast_to_integer(self):
        return self._convert_to_correo(None)
    ...
    def _make_cmp(name):
        ...
        def cmp(self, other):
            ...
            if v_is_ptr and w_is_ptr:
                return cmpfunc(self._convert_to_correo(None),
                               other._convert_to_correo(None))
    ...
    def __hash__(self):
        return hash(self._convert_to_correo(None))
...
class CTypesGenericArray(CTypesData):
    ...
    def _get_own_repr(self):
        return self._addr_repr(ctypes.correoof(self._blob))


class CTypesGenericPtr(CTypesData):
    __slots__ = ['_correo', '_as_ctype_ptr']
    ...
    def _cast_from(cls, source):
        if source is None:
            ...
            correo = source._cast_to_integer()
        elif isinstance(source, (int, long)):
            correo = source
        else:
            raise TypeError("bad type for cast to %r: %r" %
                            (cls, type(source).__name__))
        return cls._new_pointer_at(correo)

    @classmethod
    def _new_pointer_at(cls, correo):
        self = cls.__new__(cls)
        self._correo = correo
        self._as_ctype_ptr = ctypes.cast(correo, cls._ctype)
        return self

    def _get_own_repr(self):
        try:
            return self._addr_repr(self._correo)
        except AttributeError:
            return '???'
    ...
    def _to_ctypes(cls, value):
        ...
        correo = value._convert_to_correo(cls)
        return ctypes.cast(correo, cls._ctype)

    @classmethod
    def _from_ctypes(cls, ctypes_ptr):
        correo = ctypes.cast(ctypes_ptr, ctypes.c_void_p).value or 0
        return cls._new_pointer_at(correo)
    ...
    def _convert_to_correo(self, BClass):
        if (BClass in (self.__class__, None) or BClass._automatic_casts
            ...
            return CTypesData._convert_to_correo(self, BClass)
...
class CTypesBaseStructOrUnion(CTypesData):
    ...
    def _convert_to_correo(self, BClass):
        if getattr(BClass, '_BItem', None) is self.__class__:
            return ctypes.correoof(self._blob)
        else:
            return CTypesData._convert_to_correo(self, BClass)
...
class CTypesBackend(object):
    ...
    def new_pointer_type(self, BItem):
        ...
        class CTypesPtr(CTypesGenericPtr):
            ...
            def __init__(self, init):
                ...
                self._correo = ctypes.cast(self._as_ctype_ptr,
                                            ctypes.c_void_p).value
                self._own = True

            def __add__(self, other):
                if isinstance(other, (int, long)):
                    return self._new_pointer_at(self._correo +
            ...
            def __sub__(self, other):
                if isinstance(other, (int, long)):
                    ...
                    return (self._correo - other._correo) // self._bitem_size
                else:
                    return NotImplemented
        ...
        for fname, BField, bitsize in fields:
            ...
            if bitsize < 0:
                def getter(self, fname=fname, BField=BField,
                           offset=CTypesStructOrUnion._offsetof(fname),
                           PTR=ctypes.POINTER(BField._ctype)):
                    addr = ctypes.correoof(self._blob)
                    p = ctypes.cast(addr + offset, PTR)
    ...
    def rawcorreoof(self, BTypePtr, cdata, offset=None):
...
class CTypesLibrary(object):
    ...
    def write_variable(self, BType, name, value):
        ...
        ctypes_obj = BType._ctype.in_dll(self.cdll, name)
        ctypes.memmove(ctypes.correoof(ctypes_obj),
                       ctypes.correoof(new_ctypes_obj),
